 ...
 */
public class RdmbsDatabaseMeta extends BaseDatabaseMeta implements DatabaseInterface {

    private volatile static RdmbsDatabaseMeta single;

    public static RdmbsDatabaseMeta getInstance() {
        if (single == null) {
            synchronized (RdmbsDatabaseMeta.class) {
                if (single == null) {
                    single = new RdmbsDatabaseMeta();
                }
            }
        }
        return single;
